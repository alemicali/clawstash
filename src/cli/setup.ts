...
import { ensureBucket } from "../core/s3.js";
import {
  saveConfig,
  loadConfig,
  DEFAULT_RETENTION,
...
export async function setupCommand(): Promise<void> {
  ...
  if (provider === "r2") {
    storageQuestions.push(
      ...
      },
      {
        type: "select",
        name: "r2Jurisdiction",
        message: "R2 data location:",
        choices: [
          { title: "Automatic (default)", description: "Cloudflare picks the closest region", value: "" },
          { title: "European Union (EU)", description: "Data stays in EU jurisdiction", value: "eu" },
        ],
      },
    );
  }
  ...
  const storage: StorageConfig = {
    ...
    ...(storageAnswers.r2Jurisdiction && { jurisdiction: storageAnswers.r2Jurisdiction }),
  };
